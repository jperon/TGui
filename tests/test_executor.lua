...
R.describe("Executor — requêtes simples", function()
  ...
  R.it("champ null → présent en JSON null (pas clé absente)", function()
...
R.describe("Executor — erreurs", function()
  ...
  R.it("parse error → erreur retournée", function()
    ...
    R.ok(res.errors)
    return R.eq(type(res.data), 'cdata', "data doit être json.NULL en cas d'erreur")
  end)
  ...
  return R.it("champ inexistant → null JSON dans data", function()
    local json = require('json')
